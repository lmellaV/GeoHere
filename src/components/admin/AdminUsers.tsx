...
export function AdminUsers() {
  ...
  const handleAdd = async () => {
    const res = await fetch('/api/admin/users', {
      ...
      body: JSON.stringify({ username, name, password, company_id: companyId }),
    })
    const data = await res.json()
    if (data.success) { setIsAdding(false); fetchUsers(); setUsername(''); setName(''); setPassword(''); setCompanyId('') }
  ...
  const openEdit = (u: any) => {
    setSelectedUserId(u.id); setUsername(u.username); setName(u.name || '')
    setPassword(''); setCompanyId(u.companyId || ''); setIsEditOpen(true)
  }

  const handleEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); if (!selectedUserId) return; setIsSubmitting(true)
    try {
      const body: any = { username, name, company_id: companyId }
      ...
      const data = await res.json()
      if (data.success) { setIsEditOpen(false); fetchUsers() } else alert(data.message)
    ...
    <div className={s.container}>
      ...
      {isAdding && (
        <div className={s.addPanel}>
          ...
          <div className={s.formRow}>
            <input className={s.input} placeholder="RUT (ej: 12345678-9)" value={username} onChange={e => setUsername(e.target.value)} />
            <input className={s.input} placeholder="Nombre Completo" value={name} onChange={e => setName(e.target.value)} />
          </div>
          <div className={s.formRow} style={{ marginTop: '0.5rem' }}>
            <input className={s.input} type="password" placeholder="Contraseña inicial" value={password} onChange={e => setPassword(e.target.value)} />
      ...
      <div className={s.tableWrapper}>
        <table className={s.table}>
          ...
          <tbody>
            {users.length === 0 && <tr className={s.emptyRow}><td colSpan={4}>Sin usuarios</td></tr>}
            {users.map(u => (
              <tr key={u.id}>
                <td><span className={s.cellBold}>{u.name}</span></td>
                ...
                <td>
                  <div className={s.actions}>
                    <button className={`${s.btn} ${s.btnGhostBlue} ${s.btnSm}`} onClick={() => openEdit(u)} title="Editar"><Edit size={14} /></button>
                    <button className={`${s.btn} ${s.btnGhostRed} ${s.btnSm}`} onClick={() => handleDelete(u.id)} title="Eliminar"><Trash2 size={14} /></button>
                  </div>
                </td>
              </tr>
            ))}
          ...
          <div className={s.modal} role="dialog" aria-modal="true">
            ...
            <form onSubmit={handleEditSubmit}>
              <div className={s.modalBody}>
                <div><label className={s.label}>RUT / Username</label><input className={s.input} value={username} onChange={e => setUsername(e.target.value)} required /></div>
                <div><label className={s.label}>Nombre Completo</label><input className={s.input} value={name} onChange={e => setName(e.target.value)} required /></div>
